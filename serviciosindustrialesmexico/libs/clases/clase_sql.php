...
class ClaseSQL {
	...
	public function insertar(){
		...
		$this->mysqli_c->set_charset('utf8');
	...
	public function camposExistentesDB(){
		...
		if($numA == 0){
			...
			$don = trim(func_get_arg(0));
			$sql = $this->mysqli_c->query('SELECT * FROM information_schema.COLUMNS WHERE TABLE_NAME="' . $this->tabla . '" AND TABLE_SCHEMA="' . $this->basedatos . '" ' . $don);
		}
		$campos = array();
